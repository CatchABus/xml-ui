...
import { AbnormalState, compile, setAbnormalStateReceiveListener, Position } from '@nativescript-community/xml-ui-compiler';
...
async function loadContent(loader, content, options: LoaderOptions): Promise<string> {
  ...
  const { output, pathsToResolve } = compile(content, moduleRelativePath, options.platform, options.useDataBinding, options.preprocess?.attributeValueFormatter);
