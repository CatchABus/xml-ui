...
import { notifyError, notifyWarning } from '../parse/compiler-abnormal-state';
import viewCollections from '../view-collections.json';
  ...
  public onTagOpening(tagName: string): void {
    ...
    if (isViewProperty) {
      ...
      if (openTagInfo != null && openTagInfo.tagName === parentTagName) {
        const collectionProperties = Object.values(viewCollections);
        ...
        if (tagPropertyName.endsWith(KNOWN_TEMPLATE_SUFFIX)) {
          ...
          newTagInfo.astInfo.body = [];
        } else if (collectionProperties.includes(tagPropertyName)) {
          newTagInfo.type = ElementType.COLLECTION_PROPERTY;
      ...
      if (this.isClosingTag(tagName, openTagInfo)) {
        if (openTagInfo.propertyName != null) {
          switch (openTagInfo.type) {
            case ElementType.COLLECTION_PROPERTY: {
              if (openTagInfo.childIndices.length) {
                const childrenAstElements = openTagInfo.childIndices.map(childIndex => t.identifier(ELEMENT_PREFIX + childIndex));
                ...
                openTagInfo.astInfo.body.push(
                  t.expressionStatement(
                    t.assignmentExpression(
                      ...
                      t.identifier(ELEMENT_PREFIX + childIndex)
                    )
                  )
                );
              }
              ...
              openTagInfo.astInfo.body.push(
                t.expressionStatement(
                  t.callExpression(
                    t.memberExpression(
                      t.identifier('DOMHelper'),
                      t.identifier('addChildrenToParent')
                    ), [
                      t.identifier(ELEMENT_PREFIX + openTagInfo.index),
                      t.arrayExpression(childrenAstElements)
                    ]
    ...
    astBody.push(
      ...
      t.variableDeclaration('let', [
        t.variableDeclarator(
          ...
          t.callExpression(
            t.identifier('require'), [
              t.stringLiteral('@nativescript-community/xml-ui-core')
            ]
          )
        )
      ])
